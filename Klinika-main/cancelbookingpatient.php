...
<body style="background-image:url(images/cancelback.jpg)">
	...
	<div class="sucontainer">
		<label style="font-size:20px" >Wybierz wizytę do anulowania:</label><br>
		...
		$date= date('Y-m-d');
		$sql1="SELECT * from book where username='".$username."'and status not like 'Anulowane przez pacjenta' and DOV >='$date'";
		...
		?>
		<option value="<?php echo $rs["Timestamp"]; ?>"><?php echo "Pacjent: ".$rs["Fname"]." Date: ".$rs["DOV"]." -Dr.".$rs2["name"]." -Clinic: ".$rs3["name"]." -Town: ".$rs3["town"]." - Booked on:".$rs["Timestamp"]; ?></option>
		<?php
		}
		}
		}
...
if(isset($_POST['submit']))
{
		...
		$updatequery="update book set Status='Anulowane przez pacjenta' where username='$username' and timestamp= '$timestamp'";
